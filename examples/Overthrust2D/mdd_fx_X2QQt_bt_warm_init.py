...
for ifreq in range(freqsPerProcess+1):
    ...
    ia = np.load('./input/newA_npad_600_fidx_%d.npy'%idx_fname)
    ib = np.load('./input/newB_npad_600_fidx_%d.npy'%idx_fname)
    ...
    np.save('./output/newXQQt_warm_init_%d_%s_r_%d_lam_%s_lamNorm_%d_rinit_inertia_%.1f_bt'%(idx_fname, regType, args.nr, args.lam[0], args.lamNorm, args.inertia), x1)
